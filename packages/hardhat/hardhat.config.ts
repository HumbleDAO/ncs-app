...
async function addr(ethers: typeof import('ethers/lib/ethers') & HardhatEthersHelpers, addr: any) {
    if (isAddress(addr)) {
...
task('send', 'Send ETH')
    ...
    .setAction(async (taskArgs, { network, ethers }) => {
        ...
        const fromSigner: JsonRpcSigner = await ethers.provider.getSigner(from)
